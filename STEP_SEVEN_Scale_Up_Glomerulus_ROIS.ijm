macro "STEP_SEVEN_Scale_Up_Glomerulus_ROIS" {
...
ann_directories = newArray(number_of_directories);
...
for (i = 0; i < number_of_directories; i++) {
	Dialog.addMessage("SOURCE DIRECTORIES")
  	Dialog.addDirectory("CHANNEL " + (i) + " FULL RESTITCH IMAGE: ",default_path);
  	Dialog.addDirectory("CHANNEL " + (i) + " ROI OUTPUT FROM PP1: ",default_path);
}
Dialog.show();

for (i = 0; i < number_of_directories; i++) {
	source_directories[i] = Dialog.getString();
	ann_directories[i] = Dialog.getString();
}
...
annotations=getFileList(ann_directories[q]); 
number_of_files=files_to_process.length;
k=0;
source_directory_components = split(ann_directories[q], "\\"); 
...
for (aa=0; aa<number_of_files; aa++) {
	...
	for (i = 0; i < n; i++) {
		...
		rName = Roi.getName();
		roiManager("Select", i);
		RoiManager.scale(4.0, 4.0, false);
		roiManager("Select", i);
    	roiManager("Save", output_directories[q]+"\\"+rName+"_"+current_file+".roi");
	}
	run("Close All");
	roiManager("reset");
}}
