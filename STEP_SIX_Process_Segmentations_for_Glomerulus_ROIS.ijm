macro "STEP_SIX_Process_Segmentations_for_Glomerulus_ROIS" {
...
output_directories[q] = String.join(output_directory_components,"\\")+"\\pp1";
...
for (aa=0; aa<number_of_files; aa++) {
	...
	bottom_limit = 300;
	top_limit = "Infinity";
	run("Analyze Particles...", "size="+bottom_limit+"-"+top_limit+" circularity=0.25-1.00 show=[Overlay Masks] add composite");
	setForegroundColor(255, 255, 255);
	floodFill(width/4, 10);
	setForegroundColor(0, 0, 0);
	floodFill(width/4, 10);
	n = roiManager("count");
	if(n>0){
	roiManager("Save", output_directories[q]+"\\"+current_file+".zip");
